...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C =  nn.Parameter(nn.init.xavier_uniform_(torch.empty(1, config.bottleneck_dim, config.n_embd))) # context length reducing to 64 from 128 
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(1, config.block_size, config.n_embd))# position embedding (1, block_size, n_embed)
        self.drop = nn.Dropout(config.embd_pdrop)# dropout after tok_emb + pos_emb
        # transformer
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.ln_f(x)
        logits = self.head(x) #(batch_size, block_size, voca_size)) 
