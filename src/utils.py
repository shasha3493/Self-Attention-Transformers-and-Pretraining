...
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None):
    ...
    for k in range(steps):
        x_cond = x if x.size(1) <= block_size else x[:, -block_size:] # crop context if needed, we appended the nect token and tehrefore to generate the next token we just take last block size tokens as context
